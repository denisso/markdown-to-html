  ...
  return (node: Node) => {
    ...
    for (const child of node?.children || []) {
      if (
        ...
        const { codeblock, codelines, codebox } = createCodeBlockWrapperAST();

        let code;
        try {
          code = JSON.parse(JSON.stringify(child));
        } catch (err: any) {
          if (typeof err === "string") codebox.push(createBlockWithText(err));
        ...
        let blockLine = createBlockWithText("\u00A0");
        blockLine.properties = { className: ["codeline"] };
        codelines.push(blockLine);
        for (let i = 1; i <= pos.end.line - pos.start.line - 1; i++) {
          blockLine = createBlockWithText(i);
          blockLine.properties = { className: ["codeline"] };
          codelines.push(blockLine);
        }
        blockLine = createBlockWithText("\u00A0");
        blockLine.properties = { className: ["codeline"] };
        codelines.push(blockLine);
      }
    }
